...
create_simulation_update_rules_xml <- function(parent_node, updaterules) {
  updaterules_node <- xml2::xml_add_child(parent_node, "updaterules")
  if (length(updaterules) > 0) {
    for (rule in updaterules) {
      stopifnot(
        all(c("characteristicid", "classname") %in% names(rule)),
        is.numeric(rule$characteristicid),
        is.character(rule$classname)
      )

      rule_node <- xml2::xml_add_child(updaterules_node, "updaterule")
      xml2::xml_add_child(rule_node, "characteristicid", rule$characteristicid)
      xml2::xml_add_child(rule_node, "classname", rule$classname)

      if (!is.null(rule$configurationfile)) {
        xml2::xml_add_child(rule_node, "configurationfile", rule$configurationfile)
      }
    }
  }
  return(updaterules_node)
}


create_simulation_population_xml <- function(parent_node, populations) {
  if (length(populations) > 0) {
    stopifnot(is.character(populations))
    for (pop in populations) {
      xml2::xml_add_child(parent_node, "pop", pop)
    }
  }
}


create_model_configuration_xml <- function(lb,
                                           timestep,
                                           runmode,
                                           steps_between_saves,
                                           steps_in_run,
  ...
  stopifnot(
    is.numeric(timestep),
    is.character(runmode),
    is.numeric(steps_between_saves),
    is.character(steps_in_run)
  )
  ...
  # Add basic elements
  xml2::xml_add_child(root, "lb", lb)
  xml2::xml_add_child(root, "timestep", timestep)
  xml2::xml_add_child(root, "runmode", runmode)
  xml2::xml_add_child(root, "stepsbetweensaves", steps_between_saves)
  xml2::xml_add_child(root, "stepsinrun", steps_in_run)
  xml2::xml_add_child(root, "stoppingcondition", stopping_condition)

  # Add populations
  create_simulation_population_xml(root, populations)

  # Add update rules
  create_simulation_update_rules_xml(root, updaterules)
...
create_update_rule_configuration_xml <- function(char_id,
                                                 n_cat,
                                                 duration_class,
                                                 null_transition,
                                                 transition_file,
  ...
  stopifnot(
    is.numeric(char_id),
    is.numeric(n_cat),
    is.numeric(duration_class),
    is.numeric(null_transition),
    is.character(transition_file),
    is.numeric(random_seed)
  )

  schema_name <- "updateRuleConfiguration"
  ...
  xml2::xml_add_child(root, "charID", char_id)
  xml2::xml_add_child(root, "nCat", n_cat)
  xml2::xml_add_child(root, "durationClass", duration_class)
  xml2::xml_add_child(root, "nullTransition", null_transition)
  xml2::xml_add_child(root, "transitionFile", transition_file)
  xml2::xml_add_child(root, "randomSeed", random_seed)
