...
our working directory with:

```{r}
fs::dir_copy(system.file("extdata", "example-nl", package = "hiaR"), ".", overwrite = TRUE)
```

The `example-nl/Reference_Data` folder in our working directory now contains the reference data for 
...
disease_names <- get_disease_names("example-nl/Reference_Data")

cat("\nAvailable populations:\n")
population_names <- get_population_names("example-nl/Reference_Data")

cat("\nAvailable risk factors:\n")
risk_factor_names <- get_risk_factor_names("example-nl/Reference_Data")
```
...
```{r}
get_risk_factor_filenames("example-nl/Reference_Data", "Smoking_cat3")
```
...
```{r}
get_disease_filenames("example-nl/Reference_Data", "Lung_Cancer")
```
...
create_simulation_dir(
  simulation_name = file.path("example-nl/Simulations", simulation_name),
...
```
The function returns `TRUE` indicating that our configurations are valid and the sub folder was successfully created. We can look at all the available simulations with the `get_simulation_names` function:

```{r}
get_simulation_names("example-nl")
```

The sub folder will also contain the results from the simulation.
...
write(simulation_name, file.path("example-nl", batch_file_name))
```
...
(ADD: Explanation of the results and how to visualize them).
